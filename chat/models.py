...
class ChatRoom(models.Model):
    ...
    slug = models.SlugField(
        ...
        blank=True,
        max_length=255,
        help_text=_("Automatically generated from the name if left empty."),
    )
    description = models.TextField(blank=True, null=True, verbose_name="Descripción")
    is_private = models.BooleanField(default=False, verbose_name="¿Es Privada?")
    ...
    def save(self, *args, **kwargs):
        if not self.slug:
            base_slug = slugify(self.name)[: (255 - 5)]
            self.slug = base_slug
            counter = 1
