...
def parse_details_v7(session, url):
    data = {}
    try:
        ...
        # Búsqueda por clases (Estructura confirmada en HTML)
...
def process_subject(args):
    ...
    # Eliminamos el filtro de "year > 1" para ver TODO el progreso
    if not url: return subject
    
    details = parse_details_v7(session, url)
    if details:
        subject.update(details)
        # Log VERBOSE: Mostramos todo lo que encontramos
        y = subject.get('year', '?')
        print(f"   🔹 {subject['name'][:30]}... -> Año {y}")
    else:
        print(f"   ⚠️ Fallo lectura: {subject['name'][:30]}...")
    
    return subject

def main():
    print("🚀 REFINERÍA UJA V7 (VERBOSE MODE)...")
