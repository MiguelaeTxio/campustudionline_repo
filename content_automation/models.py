# /home/MiguelAeTxio/CampuStudiOnline/content_automation/models.py
...
from academic_structure.models import TimeStampedModel
...
class PendingContentTask(TimeStampedModel):
    ...
    # --- Campos para identificar el tipo de curso ---
    ...
    # --- [NUEVO] Campos para Estadísticas ---
    ...
    # --- Campos de depuración y artefactos ---
    ...
    def has_active_task(subject):
        """
        Comprueba si una asignatura ya tiene una tarea activa.
        """
        return (
...
class GeneratedContentChunk(TimeStampedModel):
    """
    Almacena una sección individual de contenido generado por la IA para una tarea específica.
    Este modelo es la piedra angular de la arquitectura de persistencia incremental,
    asegurando que el trabajo se guarde de forma atómica y se pueda reconstruir
    de manera fiable.
    """
...
class FreeContentRequest(models.Model):
    """
    Modelo para almacenar las solicitudes de contenido libre realizadas por los usuarios,
    incluyendo la lógica para el feedback de aprobación/rechazo.
    """
...
class AutomationSettings(models.Model):
    ...
    def load(cls):
        ...
        try:
            # Opción 1: Intentar obtener el objeto sin usar get_or_create para evitar caché agresivo
            obj = cls.objects.get(pk=1)
        except cls.DoesNotExist:
            # Si no existe, crearlo (solo debería pasar la primera vez)
            obj = cls.objects.create(pk=1, seed_year="")
        
        # Forzar un refresh, esencial si Celery ha modificado el campo y el proceso es de larga duración.
        obj.refresh_from_db()
        return obj
